...
migration="$ROOT/migrations/1786271923.sh"
widget="$ROOT/shell/plugins/bar/widgets/KeyboardLayout.qml"
test_dir=$(mktemp -d)
...
without_widget='del(.bar.layout[][] | select((if type == "object" then .id else . end) == "omarchy.keyboard-layout"))'
...
# The migration is only free because the widget hides itself on a machine with
# a single layout. If that guard goes, every bar gains a permanent label.
grep -q 'visible: layoutLabel !== "" && layoutCount > 1' "$widget" ||
  fail "widget stays hidden until a second layout is configured"
pass "widget stays hidden until a second layout is configured"

# ------------------------------------------------------------------ shipped default

jq -e '[.bar.layout.right[].id] | index("omarchy.keyboard-layout")' "$ROOT/config/omarchy/shell.json" >/dev/null ||
  fail "shipped config puts the keyboard layout widget in the bar"
pass "shipped config puts the keyboard layout widget in the bar"
...
[[ $(ids right) == '["omarchy.tray","omarchy.keyboard-layout","omarchy.agents","omarchy.bluetooth","omarchy.network","omarchy.audio","omarchy.monitor","omarchy.power"]' ]] ||
  fail "migration inserts the keyboard layout widget after the tray" "$(ids right)"
pass "migration inserts the keyboard layout widget after the tray"
...
write_config "$without_widget | .bar.layout.center += [{ id: \"omarchy.keyboard-layout\" }]"
run_migration

[[ $(ids center) == *'"omarchy.keyboard-layout"'* ]] || fail "migration leaves a user-placed widget alone" "$(ids center)"
[[ $(ids right) != *'"omarchy.keyboard-layout"'* ]] || fail "migration does not add a second copy" "$(ids right)"
pass "migration respects a widget the user already placed"

# Layouts written before entries grew options are bare id strings.
write_config "$without_widget | .bar.layout.right = [\"omarchy.tray\", \"omarchy.keyboard-layout\", \"omarchy.power\"]"
run_migration

[[ $(ids right) == '["omarchy.tray","omarchy.keyboard-layout","omarchy.power"]' ]] ||
...
[[ $(ids right) == '["omarchy.keyboard-layout",'* ]] || fail "migration places the widget without a tray" "$(ids right)"
